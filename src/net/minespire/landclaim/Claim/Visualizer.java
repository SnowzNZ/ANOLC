...
public class Visualizer {
        ...
        regionToCheck.getIntersectingRegions(rgManager.getRegions().values()).forEach(rg -> {
            ...
            for(int[] coord : regionBoundaries){
                if(getDistance(coord, playerPoint) < playerRadius /*&& (playerPoint[1] > coord[1] - 2 && playerPoint[1] < coord[1] + 3)*/) {
                    if(!particleCoords.contains(BlockVector3.at(coord[0], playerPoint[1], coord[1]))) regionPerimeterCoords.add(BlockVector3.at(coord[0], playerPoint[1], coord[1]));
                    //countBlocksNear++;
    ...
    public static Location getBestSpawnLocation(org.bukkit.World world, int x, int y, int z) {
        ...
        if(world.getBlockAt(x,y+1,z).getType().equals(Material.AIR)) return world.getBlockAt(x,y+1,z).getLocation();
        if(world.getBlockAt(x,y+2,z).getType().equals(Material.AIR)) return world.getBlockAt(x,y+1,z).getLocation();
        if(world.getBlockAt(x,y+3,z).getType().equals(Material.AIR)) return world.getBlockAt(x,y+1,z).getLocation();
        else return null;
    }
}
